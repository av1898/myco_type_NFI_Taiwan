...
em_dbh <- ggplot(sm_g, aes(x = EM, y = mean_dbh_large)) +
  ...
  geom_contour(aes(z = est_g), colour = "black", alpha = 0.5, binwidth = 0.1) + labs(
    y = "Mean Dbh larger trees (cm)", x = "EM proportion") + scale_fill_gradient2(
      ...
      low = "#2166AC",
      high = "#B2182B", 
      name = "Partial effect on\nlog-tree species richness") + scale_x_continuous(breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
  theme(
...
# get the difference between of EM levels in each stand develepment stage
...
## plot the effect of EM levels in young, median and late on species richness
...
pred_comparison <- ggplot(pred_effects_ll, aes
  ...
  coord_flip() +
  ylab("Change in tree species richness") +
...
Fig_2 <- em_dbh + pred_comparison + line_effect  +  plot_layout(ncol=3, widths = c(1.5, 1, 1.5))
